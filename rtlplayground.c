...
#define FRAME (((__xdata struct rtl_dot1q_frame *)&uip_buf[0]).nonq_frame)*/
	 ...
	 * then freezes the entire main loop (no timers, no HTTP, no ARP) until a
	 * power cycle. Give up after ~65k polls and drop the frame: losing one
	 * packet is recoverable, a frozen switch is not. */
	{
	uint16_t tx_guard = 0;
